...
`F-NoOwnExecution` in `judgments.md` J4 states it, with the grammar's `S-*` productions for the lint. [The specification](/typescript-as-data/spec/) is the full text and [the corpus cross-check](/typescript-as-data/conformance/corpus/) is the measurement.
