...
TypeScript. It comes with a reference implementation and a conformance suite.
...
(`@intentius/tsad-reference`). It is written from the specification text and
does not cover everything the specification defines. See
[the reference implementation](/typescript-as-data/reference/overview/) for
what it does cover and
[what it does not do](/typescript-as-data/reference/limits/).

The conformance suite is `packages/conformance`. It holds a narrow adapter
interface, the fixture format, the runner, and an adapter for chant. The
fixtures themselves live under `spec/fixtures/`, one directory per rule, so
they sit next to the text they exercise. See
[conformance](/typescript-as-data/conformance/overview/).

## The relationship to chant

[chant](https://github.com/INTENTIUS/chant) is a production implementation of
this subset, and it is where the subset came from. It is not the reference
implementation. The conformance suite tests chant, and chant's example corpus
is what the suite measures against.
...
here, with grammar, judgment, or value-domain text, an identifier and a
fixture, then implemented in chant citing that identifier, then released.
...
build opaque entities, the calls that fold together with the rule admitting
each, the tagged templates that are intrinsics, and the way an entity exposes
...
Another language would reuse the value-domain shape, the per-file decision and
identity-taint fixpoint, the two-layer admissibility structure, the three
evaluation modes, and the conformance obligations. It would have to reproduce,
rather than reuse, the AST classification, which is TypeScript's node kinds,
and ECMAScript coercion.
...
[corpus cross-check](/typescript-as-data/conformance/corpus/) is where this
