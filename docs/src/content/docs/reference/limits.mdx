...
[corpus cross-check](/typescript-as-data/conformance/corpus/) more than
anything else does. The package has no composite factory form, so
`{__compositeStep}` has no fate: its revival would be "resolve the composite,
then read `.step` off the real result", and with no factory form the package
...
has no class to become and revival rejects, which `CAVEATS.md` calls correct
rather than silent: a build that folds a resource and cannot revive it has not
...
follows it: the reference test for import and re-export captures, the entity
test for call leaks. The entry stays in `CAVEATS.md` because the code comments
cite `F-Identity` and a reader of an older revision of the specification will
not find it.

## How the gaps are accounted for

None of these gaps is allowed to be reported as a disagreement. `spec/README.md`
states the rule: a difference that a missing capability explains is counted
under a named limit and reported apart from the agreement figure, because
folding it into drift overstates what the comparison established, in the
direction that flatters the specification. Two of the corpus cross-check's four
