...
from the same host classes: own data properties, enumerable or not, accessors
skipped, because an accessor can throw and computes a value rather than holding
one. A namespace holding something with no structural encoding at all, a
function or a symbol, is reported as `not-data`, and only the verdict is
compared there.
...
at all. The first run, with no host, compared 7 files and none of them folded
...
`workflow_dispatch`. There are no pull-request or push triggers, and the
workflow explains why: the check itself takes under a second, but it can only
run against a chant checkout with its lexicon codegen artifacts generated, and
