...
Everything above assumes a JavaScript runtime is available for the files the fold refuses. That assumption is a profile, not a premise. `F-Profile` names it `full`, and `F-Profile-DataHost` names the other one (`objective.md`, spec `1.1`): the specification for an evaluator that has no runtime, embedded in a platform written in another language. `data-host` is `full` with the runtime removed. Isolation is always on and a refused file is an error rather than a demotion; J3 is absent, since nothing runs and nothing can taint; revival is serialization, since there is no constructor to call; helpers and eager intrinsics are absent; a composite is interpreted or refused, never invoked.
