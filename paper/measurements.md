...
**Codebases nobody here maintains (#129).** Every entry above was written by the people who wrote the folder. Two public projects that depend on chant and are not maintained here run beside it, at a pinned revision, each directory with a `chant.config.ts` an entry, with the imported project files a build would reach and the host a build would have at the pinned chant (`corpus-external.json`, `scripts/fetch-corpus-external.sh`). Their rows never enter the totals above.
