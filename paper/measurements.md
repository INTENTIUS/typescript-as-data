...
A file is comparable when nothing disarmed either implementation before the comparison started. At `chant-v0.70.1` four things did; at `chant-v0.71.0` two remain, and each is counted apart from the agreement figure. With F-Call implemented in the reference (#109), a host factory at a declarator is invoked and a registered project composite interpreted, and the limit that carved out 290 files is gone; what remains is 68 files that call a host factory outside declarator position, in value position or as a non-exported const's initializer, which chant invokes and J1 has no rule for (#110), and 1 that imports a package the host cannot load. Both are limits of the reference. The two that are gone were limits of chant's entry point rather than of chant: 52 files read a host data export that `foldProject` could not resolve without a lexicon list and 19 sat in entries with build parameters it could not be given, and chant#2422 gave it both (#96).

The pin moved to `chant-v0.72.0`, which carries the classifier fix for project-local calls (chant#2437), and the numbers did not move: the same 372 files agree and the `S-CallLocal` fixtures, held out of the shape half since spec `1.2`, now agree there too.

**What this establishes.** On 372 files nobody wrote for the purpose, the two implementations agree on every verdict, and on the 258 that fold on both sides the export namespaces are structurally identical, entity class and properties included. It is agreement between the two that was observed rather than designed; the second run doubled the set it is observed on and the third, with F-Call, more than doubled it again, without adding a disagreement.

**Its limit.** 372 of 441 is the comparable set, not the corpus, and the value-position limit is an over-approximation, so a file under it may also be hiding a disagreement. What it covers is one shape, and it retires with the decision on #110. chant declares spec `1.0` and the rules are at `1.1`; the policy says an addition leaves the older minor's rules unchanged, and the suite holds the declaration to the same major.

**Found on the way, third run (#109).** Implementing F-Call exposed 290 files, and three things fell out before the comparable set was clean. L6.1 lists a Declarable and a CompositeInstance among live objects and the reference's liveness test was prototype-only, so chant's composite instances, plain objects with a marker, failed step 7; the test now reads the marker the way F-Host-Interface item 1 describes it. The registration declarator `export const N = Composite(fn, "N")` is a function used as a value; the defining module therefore runs while callers interpret the body from source, which is what chant does and what one lexicon example pinned. F-Bind's destructured locals from a composite call had never been bound. The one shape left over is a host factory called outside declarator position; it is a question for the text (#110) rather than for either implementation.
