...
|---|---|---|---|
| chant `0.71.0` | 82, of which 31 are whole-build | 101 of 127 | all, on the 66 the pin can answer |
...
| `chant-v0.71.0` at `4ac9b08a`, 109 entries | 441 | 150 | 150 | 118 |

A file is comparable when nothing disarmed either implementation before the comparison started. At `chant-v0.70.1` four things did; at `chant-v0.71.0` two remain, and each is counted apart from the agreement figure. 290 files reach a composite factory call, a form the reference does not implement, and 1 imports a package the host cannot load. Both are limits of the reference. The two that are gone were limits of chant's entry point rather than of chant: 52 files read a host data export that `foldProject` could not resolve without a lexicon list and 19 sat in entries with build parameters it could not be given, and chant#2422 gave it both (#96).

**What this establishes.** On 150 files nobody wrote for the purpose, the two implementations agree on every verdict, and on the 118 that fold on both sides the export namespaces are structurally identical, entity class and properties included. It is agreement between the two that was observed rather than designed, and the second run doubled the set it is observed on without adding a disagreement.

**Its limit.** 150 of 441 is the comparable set, not the corpus, and the composite limit is an over-approximation, so a file under it may also be hiding a disagreement. That limit alone removes two thirds of the corpus, and the most common form in real projects is therefore the one the cross-check cannot yet speak to. chant declares spec `1.0` and the rules are at `1.1`; the policy says an addition leaves the older minor's rules unchanged, and the suite holds the declaration to the same major.
