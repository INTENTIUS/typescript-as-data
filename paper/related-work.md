...
Draft for #27. Rule identifiers refer to `spec/`. The order follows #31: the nearest neighbors first, the configuration languages last.
...
Heldal and Hughes treat a program as modules that can be specialized independently (PEPM 1997; TCS 248, 2000). Two problems are posed. The program arrives one module at a time, or the static data is split into data modules with one residual module each. Both concern the structure of specialization. Neither is a per-module choice between specializing and not, and the setting is a functional language without object identity. `prior-art.md` records the reading as abstract-level; the full text is #47.
