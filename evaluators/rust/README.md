...
It speaks JSON on stdin and stdout. `{"op":"version"}` answers with the specification version it declares; `shape`, `foldExport` and `foldProject` take `files`, an optional `entry` and `export`, and a `host` that is a description only, the intrinsic registry and the trust set (F-Profile-DataHost). `undefined` and a non-finite number, which JSON cannot carry, travel as `{"$tsad": …}` objects the adapter in `packages/conformance/src/adapters/rust.ts` decodes.
