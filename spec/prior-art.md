...
Where chant differs: the decision there is by a *syntactic marker the author
...
tracked through abstract values. It never falls back to running the original
per unit; it always produces one residual program.

Where chant differs: residualization is finer-grained than per-file fallback
...
(*Theoretical Computer Science* 248, 2000). Their framing: all prior partial
...
Nix evaluation is pure, but IFD pauses evaluation, realises a store object (a
build), and resumes with its contents — the community's own framing is that
...
Where chant differs: IFD escapes into a *build* of something else; chant's
...
Starlark: no side effects, no recursion, not Turing-complete. Dhall: forbids
recursion and side effects, statically typed. CUE: no general recursion, no
general functions. Jsonnet: recursion and laziness, no static types. Nickel:
Turing-complete, and — a correction to the plan's earlier framing — **permits
side effects**, constrained to be commutative and planned as an extensible
effect system.
...
- **#5, #27:** lead related work with CTFE and Heldal & Hughes, not with the
  configuration languages. Retire the sentence "every comparable system
  answers out-of-subset source with a syntax error".
- **#15:** the identity-taint fixpoint is the paper's technical centre. Its
  spec issue should be written knowing that.
- **#36:** cite Nix IFD as the precedent for evaluation escaping into a
  sandboxed execution.
- **#28:** the theorem to state is about identity across the boundary, not
  about the fallback existing.
