...
|---|---|---|---|
| L1.1 | admissible export shapes | `export const X = new Type(...)`, `export const X = <expr>`, `export const {a,b} = <expr>`, `export {a,b}`, `export {a,b} from "./m"`, `export function f(){}` | GAP (F1) |
| L1.2 | `export default` | disqualifies the file | GAP (F1) |
| L1.3 | `export * from` | disqualifies — no enumerable element list | GAP (F1) |
...
| L3.17 | eager intrinsic referenced as a bare value | rejected — "call it instead" | **GAP (new)** |
| L3.18 | method call receiver is a symbolic envelope | rejected — else `toString` would answer with the placeholder's shape | **GAP (new)** |
| L3.19 | method call, named property not a function | rejected | **GAP (new)** |
| L3.20 | `.step` narrowing | only when the callee is not already a helper, intrinsic, `FoldableFunction`, or shadowed by a const (`isUnclaimedBareCall`) | **GAP (new)** |
...
| L5.9 | `leakedIdentity` | a call returning a live object the body produced records a taint edge; one merely passed through the arguments does not | **GAP (new — identity propagates through calls, not only imports)** |
...
| L5.11 | `params` bare-specifier case | the one recognized bare import: `@intentius/chant/params` resolves against `FoldSession.buildParams` | GAP (F2) |
...
| L6.8 | `requireLiveRefs` | inside an intrinsic's or helper's arguments a `{__attrRef}` is **rejected**, because the receiver does `instanceof` checks and `WeakRef` derefs; elsewhere it passes | **GAP (new — a value's validity is position-dependent)** |
| L6.9 | composite-step args | revived with `requireLiveRefs: false` — a composite stores props rather than inspecting them | **GAP (new)** |
...
| # | Decision | Behaviour | Covers |
|---|---|---|---|
| L7.1 | rule 1 — project files only | text check on the specifier; a lexicon-published composite is deliberately never interpreted | GAP (F3) |
| L7.2 | rule 2 — `export const N = Composite(<fn>, "N")` | and `Composite` must be chant's own **in the defining module** | GAP (F3) |
| L7.3 | rule 3 — at most one parameter, bound plainly | no rest, default, nested, or array pattern | GAP (F3) |
| L7.4 | rule 4 — body shape | concise expression, or `const`s then a final `return`; empty body rejected; **must end in `return`** (unlike L5.7) | GAP (F3), and the L5.7 asymmetry is **GAP (new)** |
| L7.5 | rule 5 — every expression in the subset | extended with `new` in value position and calls through a bare identifier | GAP (F3) |
| L7.6 | module-level resource reference declines | that resource is a singleton the run path shares; interpretation would not | GAP (F3) |
| L7.7 | `constResolvesToResource` follows alias chains | `const a = new T(); const b = a;` cannot smuggle one in | **GAP (new)** |
| L7.8 | `MAX_INTERPRETATION_DEPTH` | self-referential composite terminated | **GAP (new)** |
...
| L8.3 | `foldModule` vs `tryFoldFile` | `foldModule` is **per-export** with an ok/false entry each and silently skips non-`new` exports; `tryFoldFile` is per-file | GAP (F7), and the silent skip is **GAP (new)** |
...
| L9.2 | trust arm 1, subpath | package root extracted from text and matched against the same set | **GAP (new)** |
| L9.3 | trust arm 2 | specifier **resolved**, path checked against chant-core's own tree — text is explicitly not enough | R2.1 |
| L9.4 | no lexicon list supplied | arm 1 disabled entirely rather than loosened | **GAP (new)** |
| L9.5 | sandboxed refusal | a fold needing an untrusted import is demoted to run; not an error | R2.2 |
| L9.6 | bare-specifier resolution cache is process-wide | documented as unsound for nested `node_modules` with a version override; the same assumption bundlers make | **GAP (new — a stated, accepted unsoundness)** |

## L10 — Observables and side outputs

| # | Decision | Behaviour | Covers |
|---|---|---|---|
| L10.1 | `FoldExecutionCounts` | `factoryInvocations`, `projectFactoryInvocations`, `factoryInterpretations` — process-wide, monotonic, resettable | GAP (F10) |
